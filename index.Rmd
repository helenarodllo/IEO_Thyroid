...
You also need some other packages, in order to run all the analysis. These are `r Biocpkg("SummarizedExperiment")`, `r Biocpkg("edgeR")`, `r Biocpkg("geneplotter")`, `r Biocpkg("sva")` and `r Biocpkg("corpcor")`.   
...
cat(readLines("./QA_analysis.Rmd"), sep="\n")
```

```{r echo=FALSE, results='asis'}
cat(readLines("./DE_analysis.Rmd"), sep="\n")
```

```{r echo=FALSE, results='asis'}
cat(readLines("./FEanalysis.Rmd"), sep="\n")
```
