...
With this filter, we have a dataset of 20115 genes and 467 samples. This way, none of our samples has a low sample depth. 
...
```{r distRawExp, echo=FALSE, fig.height=4, fig.width=12, out.width="800px", fig.cap="Fig3. Box plot of the distribution expression values across samples", message=FALSE}
...
In the blox plot, we can observe there are no samples that deviate a little bit of the average. For this reason, we assume that we do not need to normalise samples. 
