...
First of all, the number of significant differential expressed genes is calculated in \@ref(pvalue), this is done without any batch adjustment. The FDR approach is used to perform this calculation as some false positives can be accepted in this studie, the aim is to be more laxe.
...
```{r pdist, echo=FALSE, fig.height=6, fig.width=12, out.width="800px", fig.cap="Distribution of raw p-values for an F-test on every gene between tumor and normal samples."}
...
`r sv$n` surrogate variables are found, so the extent of differential expression is assessed adjusting for them \@rev(batchadjust).
...
```{r psvdist, echo=FALSE, fig.height=6, fig.width=12, out.width="800px", fig.cap="Distribution of raw p-values for an F-test on every gene between tumor and normal samples, adjusting for surrogate variables estimated with SVA."}
...
```{r sampleClusteringAdjust, fig.height=6, fig.width=12, out.width="800px", echo=FALSE, fig.cap="Hierarchical clustering of the samples after adjusting for surrogate variables"}
...
It can be observed that there is still two clusters due to the surrogate variable, samplevial. The batch adjustement by sva had no effect for this variable, so we are going to use an agressive strategy called *Singular Value Decomposition (SVD)* in order to remove this batch effect \@ref(svd). Results would be again visualized with a Hierarchical Clustering \@ref(fig:sampleClusteringRemov). 
...
```{r sampleClusteringRemov, fig.height=6, fig.width=12, out.width="800px", echo=FALSE, fig.cap="Hierarchical clustering after removing samplevial effect."}
...
```{r VolcanoPlot, fig.height=8, fig.width=12, out.width="800px", echo=FALSE, message=FALSE, fig.cap="Volcano Plot of normalized genes."}
