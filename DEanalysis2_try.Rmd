...
fit$genes <- genesmd
DEgenes1 <- topTable(fit, coef = 2, n = Inf, lfc = 2.5)
head(DEgenes1)
...
limma::plotMA(fit, coef = 2, status = rownames(fit$lods) %in% DEgenes1, legend = FALSE,
...
summary(res2)
fit2$genes <- genesmd
DEgenes2 <- topTable(fit2, coef = 2, n = Inf, lfc = 2.5)
head(DEgenes2)
...
summary(res3)
fit3$genes <- genesmd
DEgenes3 <- topTable(fit3, coef=2, n=Inf, lfc = 2.5)
head(DEgenes3)
...
mod <- model.matrix(~type + sample_id, colData(thca.paired))
...
including sample_id in the model as are paired samples we get 20 up and 4 down-regulated genes.
